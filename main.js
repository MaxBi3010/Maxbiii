...
global.img = require("./utils/api_img.json");
...
function onBot({ models }) {
  ...
  try {
    ...
    console.log('appState đã được lấy từ cookie');
  } catch (error) {
...
const globalPath = path.join(__dirname, './data_api/datajson/global.gaudev.json');
let globalData = fs.existsSync(globalPath) 
    ? JSON.parse(fs.readFileSync(globalPath)) 
    : { vdsp: [], anime: [], trai: [] };

let vdsp = globalData.vdsp;
let anime = globalData.anime;
let trai = globalData.trai;
...
setInterval(async () => {
    if (toggle === 0) {
        let urls = require('./data_api/datajson/gaivjp.json');
        await processUpload(vdsp, urls);
    } else if (toggle === 1) {
        let animeUrls = require('./data_api/datajson/anime.json');
        await processUpload(anime, animeUrls);
    } else if (toggle === 2) {
        let traiUrls = require('./data_api/datajson/vdtrai.json');
        await processUpload(trai, traiUrls);
    } 
    toggle = (toggle + 1) % 3;
}, 1000 * 5); 
